...
// Simulate the model (on 500 periods, 500 different simulations => those simulations are stored in a file "main_simul", to retrieve if needed)
...
beta_range = [0.25 0.5 0.65 0.7 0.75 0.8 0.85 0.99];
// jscale_range = [0.4111 0.32926 0.30197 0.32862 0.57917 0.32797 NULL NULL 0.34148]
results = struct();
 for i=1:length(beta_range)
    ...
    // 0.57917 (beta = 0.75); 0.32797 (beta = 0.80); 0.34953 (beta = 0.90); 0.42157 (beta = 0.95); 0.34148 (beta = 0.99)]
